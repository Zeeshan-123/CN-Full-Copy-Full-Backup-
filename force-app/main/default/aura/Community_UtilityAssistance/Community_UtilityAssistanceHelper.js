({
    ...
    addHHMemberRecord: function(component, event, helper){
        ...
        if( HHMembersList.length < 15){
            ...
            HHMembersList.push({
                ...
                'Gross_Weekly_Wages__c' : 0, 
                'Welfare_TANF__c' : 0,
                'Social_Security_Benefits__c': 0,
                'Unemployment_Benefits__c': 0, 
                'All_Other_Income__c': 0, 
                'Certified_No_Income__c' : false
            });
            component.set("v.HHMembersList", HHMembersList);
            var leng = HHMembersList.length - 1; 
           // component.set("v.activeSections", leng)
        }
        else{
    ...
    addVendorRecord: function(component, event, helper){
        //component.set("v.HHMembersRequired", true);
        //get the HHMembersList List from component  
        var VendorList = component.get("v.vendorsList");
        ...
        if( VendorList.length < 15){
            ...
            component.set("v.vendorsList", VendorList);  
        }
        else{
            helper.showErrorMessage(component,event,helper,'A maximum of 15 Vendor records can be added.');
        }
    },
    ...
    handleSearch: function( component, searchTerm ) {
        var action = component.get( "c.searchVendors" );
       action.setParams({
        ...
        action.setCallback( this, function( response ) {
            ...
            var event = $A.get( "e.c:customLookupEvt" );
            event.setParams({
                "vendorLookup": response.getReturnValue()
            });
