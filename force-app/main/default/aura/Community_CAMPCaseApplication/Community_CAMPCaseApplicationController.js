({
        ...
        action.setCallback(this,function(response){
            ...
            if (state === "SUCCESS") {
                ...
                if (resultData.ClothingVoucherApp != undefined) {
                    ...
                    if(age > 21 ){
                        component.set("v.showAgeResr", true);
                    }else{
                        ...
                        var controlselectAuraIds1 = ["PhysicalStreetInput", "PhysicalAddress2Input", "PhysicalSuiteInput", "PhysicalCityInput", "PhysicalStateInput", "PhysicalzipInput",
                                                     "priConCellPhone", "SecConFirstName", "SecConLastName", "SecConPhone", "SecConRelation"];
                        var age = helper.getAge(component, event, resultData.acnt.Date_of_Birth__c);
                        console.log(age);
                        if(age < 22  ){
                            ...
                            if(age < 18  ){
                                component.set("v.Signedinunder18", true);
                            }else{
                                component.set("v.Signedinunder18", false);
                            }
                        }
    ...
    validateSchoolName:  function(component, event)   { 
        var regex = new RegExp("[^a-zA-Z- ]","g");
        var fieldId = event.getSource().getLocalId();
        var fieldInput = component.find(fieldId);
        var inputValue = fieldInput.get('v.value');
    ...
    validateTextArea:  function(component, event)   { 
        debugger;
    ...
    //format GPA fields
    formatGPA:  function(component, event)   { 
        debugger;
        var fieldId = event.getSource().getLocalId();
        var fieldInput = component.find(fieldId);
        ...
        var gPAVal = val.replace(re,"");
        var inputValue=gPAVal;
        var gPAValFirstCharacter = gPAVal.charAt(0);
        if (( gPAVal > 40 ) &&  ( gPAVal < 50 ) || ( gPAVal > 400 ) &&  ( gPAVal < 500 )){
            inputValue="4.00";
            ...
            if (( gPAVal >= 0 ) &&  ( gPAValFirstCharacter < 5 ) ){
                ...
                if( gPALen > 1){
                    var gPAa=gPAVal.slice(0,1);
                    var gPAb=gPAVal.slice(1);
                    inputValue = gPAa + "." + gPAb; 
                    fieldInput.set('v.value', inputValue);
                }else{ 
                    inputValue=gPAVal;
                    fieldInput.set('v.value', inputValue);
    ...
    handleAddressClick: function(component, event, helper)  {
        debugger;
         component.set("v.addressClicked", true);
    ...
    handleToggleChange: function(component, event, helper){
        var  addressClicked = component.get("v.addressClicked");
        var  CVCaseClone = component.get("v.CVCaseClone");
        var melissaResult = component.get("v.predictions");
        var states = component.get("v.states");
            ...
            if(melissaResult[i].Address.SuiteCount > 0 ){
                for (var j = 0; j < melissaResult[i].Address.SuiteCount; j++){
                    if((melissaResult[i].Address.AddressLine1+' '+melissaResult[i].Address.SuiteList[j]+', '+
                        ...
                        component.set("v.CVCase.Physical_Address2__c", '');
                        component.set("v.CVCase.Is_Physical_Address_Changed__c", true);
                    }
                }
            } 
            if(melissaResult[i].Address.SuiteCount < 1){
    ...
    handlePhysicalToggleChange: function(component, event, helper){
        ...
        if(!component.get("v.isPAManual")){
            ...
            if(melissaResult.length > 0 && addressClicked) {
                ...
                component.set("v.CVCase.Physical_States__c", CVCaseClone.Physical_States__c); 
                component.set("v.CVCase.Physical_Countries__c", CVCaseClone.Physical_Countries__c); 
                component.set("v.CVCase.Is_Physical_Address_Changed__c", false);
            }
        }
        else  {
    ...
    reviewInformation: function(component, event, helper){
        ...
        var SecConInfoRequired = true;
        var StudentInfoRequired = true;
        ...
        if(isValidAddress && isValidPhyAddress){
            ...
            if(showPrimaryGuardianSection)
                controlselectAuraIds.push("PriGuardianFirstName", "PriGuardianLastName", "PriGuardianPhone","PriGuardianEmail");
            if(SecConInfoRequired)
                controlselectAuraIds.push("SecConFirstName", "SecConLastName", "SecConPhone", "SecConRelation");
            if(StudentInfoRequired)
                controlselectAuraIds.push("StTShirtSize", "StHighSchool", "StGrade1", "StGPA","StCollegeCareer","Strolestitlesheld");
        ...
        if (isAllValid && isLookupValSelected && isPhyLookupValSelected && isValidAddress && isValidPhyAddress && isAddressDataValid && isPhyAddressDataValid){
            ...
            if(showPrimaryGuardianSection){
                var pgEmail = CVCase.PG_Email__c;
                
                var mailformat = /^[a-zA-Z0-9._-]+@[a-z0-9\-]+\.[a-z]{2,3}$/;
                if(pgEmail.match(mailformat))
                {}else
                {    isMobileFieldsDataValid = false;
                 phoneFieldError +='You have entered an invalid email address.\n';
                }
            ...
            if(CVCase.Sec_First_Name__c  != '' &&  CVCase.Sec_Last_Name__c  != ''  && CVCase.Sec_Phone__c  != '' && CVCase.Relation__c != ''){
                ...
                if(secConPhone.length != 10) {
                    isMobileFieldsDataValid = false;
                    phoneFieldError +='Mobile number must contain 10 digits only for Emergency Contact.\n';
                }
                if(firstCharValforSecCon == '1'  || firstCharValforSecCon == '0') {
                    isMobileFieldsDataValid = false;
                    phoneFieldError +='Mobile number should not begin with 1 or 0 for Emergency Contact.\n';
                }
            }
                    ...
                    helper.showErrorMessage(component, event, helper, phoneFieldError);
                        ...
                        helper.showErrorMessage(component, event, helper,'There are invalid characters in the Physical Address fields. Please provide a valid Physical Address.');
                            ...
                            helper.showErrorMessage(component, event, helper, 'Must check the I have verified that my physical address is correct checkbox.');
                                ...
                                helper.showErrorMessage(component, event, helper, 'At least one file must be uploaded to letter of Recommendation mentioned.');
        
                                else if(incomeUploadedFilesList.length >= 6)
                                    helper.showErrorMessage(component, event, helper, 'You cannot uploaded more than 5 files for letter of Recommendation file upload section.');
        
                                    else if(custodyUploadedFilesList.length == 0 || custodyUploadedFilesList == '')
                                        helper.showErrorMessage(component, event, helper, 'At least one file must be uploaded to Current Photo mentioned.');
        
                                        else if(custodyUploadedFilesList.length >= 6 && custodyUploadedFilesList.length != 0)
                                            helper.showErrorMessage(component, event, helper, 'You cannot uploaded more than 5 files for Current Photo file upload section.');
        
                                            else{
                                                debugger;
                                                if (component.find("termsAndConsstu") && !component.get("v.iAcceptStu")){
                                                        helper.showErrorMessage(component, event, helper, 'Must check the I Accept checkbox for students.');
                                                        $A.util.addClass(component.find("termsAndConsstu"), 'slds-has-error');
                                                        component.set("v.iAccept", false);
    ...
    save: function(component, event, helper) {
        debugger;
        console.log(component.get("v.isPAManual"));
        console.log(component.get("v.isManual"));
        var incomeUploadedFilesList = component.get("v.incomeUploadedFilesList");
        var custodyUploadedFilesList = component.get("v.custodyUploadedFilesList");
