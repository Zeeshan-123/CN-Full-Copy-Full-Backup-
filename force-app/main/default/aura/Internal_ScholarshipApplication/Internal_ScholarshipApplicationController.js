({
        ...
        action.setCallback(this,function(response){
            ...
            if (state === "SUCCESS") {
                ...
                if (resultData.ScholarshipApp != undefined) {
                    ...
                    numOfRequests = caseList.length; 
                    if(caseList.length === 1){
                        debugger;
                    
                    component.set("v.caseId", caseList[0].Id);
                        debugger; 
                       caseNum+='Confirmation Number: ' + caseList[0].CaseNumber; 
                        debugger; 
                   ...
                   applicConcat = caseList[0].Student_Type__c + ' ' + caseList[0].Scholarship_Application__c; 
                       ...
                       applicConcat = caseList[0].Student_Type__c + ' ' + caseList[0].Degree__c + ' ' + caseList[0].Scholarship_Application__c; 
                        ...
                        confMess+='You have successfully submitted your 2022-23 CN ' + applicConcat +  ' Scholarship application.'
               component.set("v.applicationResult", confMess);
					...
					for (var c in caseList){
                       ...
                       caseNum+='<a href="/lightning/r/Case/' + caseList[c].Id + '/view" > Confirmation Number: ' + caseList[c].CaseNumber + '</a><br>'; 
                        component.set("v.requestNumber", caseNum);
                        var applicConcat = '';
                   ...
                   applicConcat = caseList[c].Student_Type__c + ' ' + caseList[c].Scholarship_Application__c; 
                       ...
                       applicConcat = caseList[c].Student_Type__c + ' ' + caseList[c].Degree__c + ' ' + caseList[c].Scholarship_Application__c; 
                        ...
                        confMess+='You have successfully submitted your 2022-23 CN ' + applicConcat +  ' Scholarship application.<br><br>' 
                    }                        
                        
                        component.set("v.numOfRequests", numOfRequests);
               component.set("v.applicationResult", confMess);
                    // show success message
    ...
    formatSSN:  function(component, event)   { 
        ...
        // Strip all characters from the input except digits
        var inputValue = val.replace(/[^X\d]/g,'');
    ...
    reviewInformation: function(component, event, helper){
        ...
        var fileError = false; 
        var fileErrorMsg = '';
        console.log('Vendor Entry: ' + CVCase.Vendor__c);
        ...
        if(isValidAddress){
            debugger; 
            var controlselectAuraIds = ["priConCellPhone", "actScore", "ssn", "studentId", "countyID", "districtID", "applicationType", "univName", "noHours", "studentClassification", "expectedMonth", "expectedGradYear", "background"];
    ...
    //incomeUploadedFilesList
      ...
      var SSNComp = component.find('ssn');
        console.log('SSN Changed: ' + ssnChanged);
        debugger; 
            var firstCharValforSSN = SSN.charAt(0);
        	var matches = SSN.includes('X');
			console.log(matches);
            if(SSN.length != 11 && ssnChanged == true) {
                errorExist = true; 
  ...
  //SSNComp.reportValidity(); 
    ...
    else if (matches != false && ssnChanged == true) {
    				errorExist = true;
        $A.util.addClass(SSNComp, 'slds-has-error');
			}
        ...
        if(!sfIDRegex.test(CVCase.Vendor__c)){
            ...
            $A.util.addClass(component.find('univName'), 'slds-has-error');
            errorExist = true; 
        ...
        action.setCallback(this,function(response) {
           ...
           if(state === 'SUCCESS') {
               ...
               component.set("v.caseId", result.Id);
                       caseNum+='<a href="/lightning/r/Case/' + result.Id + '/view" > Confirmation Number: ' + result.CaseNumber + '</a><br>'; 
                        component.set("v.requestNumber", caseNum);
                        var applicConcat = '';
